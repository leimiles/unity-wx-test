#ifndef DINO_CHARACTER_LIGHTING_INCLUDED
#define DINO_CHARACTER_LIGHTING_INCLUDED
...
void MiniLightingGeneralWithAdditionalLight(BRDFData brdfData, InputData inputData, Light light, half ndotv, half3 goochDark, half3 goochBright, half shadowIntensity, half shadowThreshold, half shadowSmooth, 
    out half3 outDiffuse, out half3 outSpecular)
{
    ...
    half3 gooch;
    gooch = lerp(goochDark, goochBright, max(diffuse.r, max(diffuse.g, diffuse.b)));
    
    diffuse = LinearStep( shadowThreshold - shadowSmooth, shadowThreshold + shadowSmooth, diffuse);
    diffuse = lerp(diffuse * 0.5 + 0.5, 1, shadowIntensity);
    // diffuse = lerp(diffuse * light.shadowAttenuation * 0.5 + 0.5, 1, shadowIntensity);
    // diffuse = lerp(light.shadowAttenuation * 0.5 + 0.5, 1, shadowIntensity);
    gooch *= diffuse;

    #if defined(LIGHTMAP_ON)
        outDiffuse = 0;
    #else
    outDiffuse = gooch * light.color;
...
// Character Use this
half3 CalculateCharacterBlinnPhong(Light light, InputData inputData, SurfaceData surfaceData, half3 goochDark, half3 goochBright)
{
    half3 attenuatedLightColor = light.color * (light.distanceAttenuation * light.shadowAttenuation);
    half3 lightDiffuseColor = 0;

    half ndotl = max(dot(inputData.normalWS, light.direction), 0.0h);
    half3 diffuse = ndotl * (light.distanceAttenuation * light.shadowAttenuation);

    half3 gooch;
    gooch = lerp(goochDark, goochBright, max(diffuse.r, max(diffuse.g, diffuse.b)));
    lightDiffuseColor = gooch * light.color;
...
half4 CharacterBlinnPhong(InputData inputData, SurfaceData surfaceData, half3 goochDark, half3 goochBright)
{
    ...
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
        #endif
    {
        lightingData.mainLightColor += CalculateCharacterBlinnPhong(mainLight, inputData, surfaceData, goochDark, goochBright);
    }
    ...
    if (IsMatchingLightLayer(light.layerMask, meshRenderingLayers))
        #endif
    {
        lightingData.additionalLightsColor += CalculateCharacterBlinnPhong(mainLight, inputData, surfaceData, goochDark, goochBright);
    }
    LIGHT_LOOP_END
    #endif
